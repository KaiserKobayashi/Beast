# PowerShell setup script: no bashisms / no <<< / no <<EOF
...
# --- Option A: use a temp file (most reliable on Windows) ---
$tmp = [System.IO.Path]::GetTempFileName()
Set-Content -LiteralPath $tmp -Value $code -Encoding UTF8
& $PYEXE $tmp
$exit = $LASTEXITCODE
Remove-Item -LiteralPath $tmp -ErrorAction SilentlyContinue
if ($exit -ne 0) { throw "One or more imports failed. See output above." }

# --- Option B (alternative): pipeline stdin (uncomment to try) ---
# $code | & $PYEXE -
